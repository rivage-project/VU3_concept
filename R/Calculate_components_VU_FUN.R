...
adaptive_capacityFUN <- function(ac_df,
  ...
  adaptive_capacity <- ac_df_norm$ac_biotic + ac_df_norm$ac_abiotic
    ...
    bootstrap_df <-
      pbapply::pblapply(1:n_samples, function(i){
        ...
        compo <- compo |>
          ...
          dplyr::mutate(Vu_rank_prod = dplyr::dense_rank(Vu_prod))
