create_df_birds <- 
  function(root_dir){
    ...
    ias <- readRDS(paste0(grand_parent_dir, "/data/derived-data/22_IAS_exposure_39_isl.rds"))
    cc <- readRDS(paste0(grand_parent_dir, "/data/derived-data/21_CC_SED_exposure_55_isl.rds"))
    lu <- readRDS(paste0(grand_parent_dir, "/data/derived-data/20_LU_exposure_55_isl.rds"))
    colnames(lu)[colnames(lu)=='ID'] <- 'ULM_ID'
    colnames(ias)[colnames(ias)=='ID'] <- 'ULM_ID'
    colnames(cc)[colnames(cc)=='ID'] <- 'ULM_ID'
    
    ac_elev <- readRDS(paste0(grand_parent_dir, "/data/derived-data/30_extrinsic_AC_elevation.rds"))
    ac_PA <- readRDS(paste0(grand_parent_dir, "/data/derived-data/30_extrinsic_AC_prop_PA.rds"))
    ac_intr <- readRDS(paste0(grand_parent_dir, "/data/derived-data/31_AC_intrinsic_BMP.rds"))
    
    threats <- dplyr::left_join(
      lu %>% dplyr::select(ULM_ID, mean_HM_static_2017, mean_HM_change, rdens_osm) %>%
        dplyr::mutate(ULM_ID = as.integer(ULM_ID)),
      dplyr::left_join(
    ...
    birds <- readRDS(paste0(grand_parent_dir, "/data/derived-data/12_bird_ckl_islands_clean.RDS"))
    
    # load traits
    tr_birds <- readRDS(paste0(grand_parent_dir, "/data/derived-data/12_Bird_traits.RDS"))
